...
var BufferExtender = require('./Buffer');

/**
 * Represents an image with Exif information. 
 */
function ImageExif(file) {
...
ImageExif.get = function (file, fn) {

    if (fn) { //异步

        var self = new ImageExif(file);
        self.loadImage(fn);
        return;
    }
    ...
    try { //同步
        ...
        var data = self.loadImageSync();

        delete data.image['Padding'];
        delete data.exif['Padding'];
...
ImageExif.prototype = { //实例方法

    constructor: ImageExif,
    ...
    loadImage: function (fn) {
        ...
        if (image.constructor.name === 'String') {
            fs.readFile(image, function (error, data) {
                ...
                var exif = self.processImage(data);
                fn(null, exif);
            });
        }
        else if (image.constructor.name === 'Buffer') {
            var exif = this.processImage(image);
            fn(null, exif);
        }
        else {
            var error = new Error('Given image is neither a buffer nor a file, please provide one of these.');
            fn(error, null);
        }

    },
    
    loadImageSync: function () {
    ...
    processImage: function (data) {
        ...
        if (data[offset++] == 0xFF && data[offset++] == 0xD8) {
            ...
            throw new Error('The given image is not a JPEG and thus unsupported right now.');
        }
        ...
        throw new Error('No Exif segment found in the given image.');
    ...
    extractExifData: function (data, start, length) {
        ...
        for (var i = 0; i < numberOfEntries; i++) {
            var exifEntry = self.extractExifEntry(data, (ifdOffset + 2 + (i * 12)), tiffOffset, this.isBigEndian, ImageExif.TAGS.exif);
        ...
        // it if available
        if (typeof this.exifData.image[ImageExif.TAGS.exif[0x8769]] != "undefined") {

            ifdOffset = tiffOffset + this.exifData.image[ImageExif.TAGS.exif[0x8769]];
            numberOfEntries = data.getShort(ifdOffset, this.isBigEndian);
        ...
        // it if available
        if (typeof this.exifData.image[ImageExif.TAGS.exif[0x8825]] != "undefined") {

            ifdOffset = tiffOffset + this.exifData.image[ImageExif.TAGS.exif[0x8825]];
            numberOfEntries = data.getShort(ifdOffset, this.isBigEndian);

            // Each IFD entry consists of 12 bytes which we loop through and extract
            // the data from
            for (var i = 0; i < numberOfEntries; i++) {
                var exifEntry = self.extractExifEntry(data, (ifdOffset + 2 + (i * 12)), tiffOffset, this.isBigEndian, ImageExif.TAGS.gps);
        ...
        if (typeof this.exifData.exif[ImageExif.TAGS.exif[0xA005]] != "undefined") {

            ifdOffset = tiffOffset + this.exifData.exif[ImageExif.TAGS.exif[0xA005]];
            numberOfEntries = data.getShort(ifdOffset, this.isBigEndian);
        ...
        if (typeof this.exifData.exif[ImageExif.TAGS.exif[0x927C]] != "undefined") {

            // Check the header to see what kind of Makernote we are dealing with
            if (this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 7) === "OLYMP\x00\x01" || this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 7) === "OLYMP\x00\x02") {
                this.extractMakernotes = require('./makernotes/olympus').extractMakernotes;
            } else if (this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 7) === "AGFA \x00\x01") {
                this.extractMakernotes = require('./makernotes/agfa').extractMakernotes;
            } else if (this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 8) === "EPSON\x00\x01\x00") {
                this.extractMakernotes = require('./makernotes/epson').extractMakernotes;
            } else if (this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 8) === "FUJIFILM") {
                this.extractMakernotes = require('./makernotes/fujifilm').extractMakernotes;
            } else if (this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 9) === "Panasonic") {
                this.extractMakernotes = require('./makernotes/panasonic').extractMakernotes;
            } else if (this.exifData.exif[ImageExif.TAGS.exif[0x927C]].getString(0, 5) === "SANYO") {
